...
const ProductDetailsAdmin = ({ productDetail, getProductDetail, getSidebar }) => {
				...
				<Content
					...
					<Row gutter={24}>
						<Col xs={24} sm={12} md={18} lg={18} xl={6}>
							<Image width={200} src={product?.img[0]} />
						</Col>
						...
						<Col xs={24} sm={12} md={18} lg={18} xl={18}>
							<Descriptions title={product?.name}>
								...
								<Descriptions.Item label='New'>{product?.news ? 'Hot' : 'New'}</Descriptions.Item>
								<Descriptions.Item label='Rating'>
									<Rate tooltips='Rating' value={product?.rate} />
								</Descriptions.Item>
								<Descriptions.Item label='Description'>{product?.description}</Descriptions.Item>
							</Descriptions>
						</Col>
					</Row>
					<a href type='link' onClick={handleClick} className='backToList'>
						{' '}
